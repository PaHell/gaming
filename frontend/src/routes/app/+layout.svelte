<script lang="ts">
	...
	let searchResults = $state<SymbolLookup | undefined>();
				...
				<div class="ml-auto flex items-center gap-2">
					<div class="flex flex-col">
						<InputGroup.Root bind:ref={searchTrigger}>
							...
							{#if searchResults?.count ?? 0 > 0}
								<InputGroup.Addon align="inline-end"
									>{searchResults?.count ?? 0} results</InputGroup.Addon
								>
							{/if}
						</InputGroup.Root>
						<div bind:this={searchContent} class="relative h-0" class:hidden={!searchOpen}>
							...
							>
								{#each searchResults?.result ?? [] as result}
									<a
								...
								{/each}
								{#if isSearching}
									<div class="px-2 py-1 text-sm text-muted-foreground">Searching...</div>
								{:else if !searchValue.length}
									<div class="px-2 py-1 text-sm text-muted-foreground">Type to search.</div>
